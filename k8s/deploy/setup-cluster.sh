...
# FIX 1: Chờ cert-manager webhook sẵn sàng trước khi tiếp tục
...
# FIX 2: Chờ opentelemetry-operator webhook sẵn sàng trước khi deploy collector
...
# FIX 3: Không set database.password trực tiếp vào values file (bị Grafana chart chặn)
# Thay vào đó chỉ set hostname và username vào values, password truyền qua --set-string lúc helm install
...
# Dùng --set-string để truyền password trực tiếp, không ghi vào file values
...
# FIX 4: Sửa typo "hotname" → "hostname"
